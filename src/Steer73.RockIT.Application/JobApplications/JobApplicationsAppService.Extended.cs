...
namespace Steer73.RockIT.JobApplications
{
	public class JobApplicationsAppService : JobApplicationsAppServiceBase, IJobApplicationsAppService
    {
        ...
        {
            var normalizedEmail = input.EmailAddress?.Trim();
            if (!string.IsNullOrWhiteSpace(normalizedEmail))
            {
                var emailKey = normalizedEmail.ToLowerInvariant();
                var alreadyApplied = await _jobApplicationRepository.AnyAsync(
                    x => x.VacancyId == input.VacancyId && x.EmailAddress.ToLower() == emailKey);

                if (alreadyApplied)
                {
                    var vacancyForContact = await _vacancyRepository.GetAsync(input.VacancyId);
                    var coordinator = await _identityUserRepository.FindAsync(vacancyForContact.IdentityUserId);
                    ...
                    var coordinatorPhone = coordinator?.PhoneNumber;

                    var details = !string.IsNullOrWhiteSpace(coordinatorEmail)
                        ? $"{coordinatorName} ({coordinatorEmail}{(string.IsNullOrWhiteSpace(coordinatorPhone) ? "" : $", {coordinatorPhone}")})"
                        : coordinatorName;

                    throw new UserFriendlyException(
                        $"You have already applied for this role using this email address. If you wish to update your details, please contact {details}.");
                }
            }
