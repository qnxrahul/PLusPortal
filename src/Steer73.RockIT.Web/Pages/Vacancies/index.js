$(function () {
    ...
    $("body").on("click", ".copy-vacancy-url", function () {
        const url = $(this).data("url");
        navigator.clipboard.writeText(url)
            .then(() => abp.notify.success(l("UrlCopied")))
            .catch(() => abp.notify.error(l("CopyFailed")));
    });
    ...
    var dataTableColumns = [
        ...
        },
        {
            ...
            render: function (data, _type, row) {
                if (data)
                {
                    if (row.identityUser.surname)
                    {
                        return data + " " + row.identityUser.surname;
                    }

                    return data;
                }

                if (row.identityUser && row.identityUser.surname) {
                    return row.identityUser.surname;
                }
        ...
        {
            data: "vacancy.groupNames",
            render: function (groupNames) {
        ...
        { data: "vacancy.projectId" },
        {
            ...
            <div class="d-flex align-items-center">
                ...
                <button class="btn btn-sm btn-secondary copy-vacancy-url"
                        data-url="${url}">
                    <i class="fa fa-copy"></i> ${l("CopyUrl")}
                </button>
            </div>`;
            }
        }
    ...
    var dataTable = $("#VacanciesTable").DataTable(abp.libs.datatables.normalizeConfiguration({
        ...
        scrollCollapse: true,
        order: [[3, "desc"]],
        ajax: abp.libs.datatables.createAjax(vacancyService.getList, getFilter),
